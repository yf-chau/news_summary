...
- **Sleep/wake:** No scheduled `pmset` wake is needed. The self-hosted runner holds a persistent connection and wake-on-network (`pmset womp 1`, on AC) brings the Mac out of sleep so jobs get picked up without a scheduled wake — though pickup is not always immediate: run `31848809059` sat 12 min until the Mac's own `rtc/Maintenance` DarkWake, which is how it ended up starting *inside* a DarkWake window.
- **Sleep lock covers the whole job.** The first step backgrounds `caffeinate -s -t 5400` (PreventSystemSleep) and records its PID in `$GITHUB_ENV`; a final `if: always()` step kills it. Every step must be covered, not just the pipeline: `caffeinate -s` was originally on the `Run pipeline` step alone, and run `31848809059` was killed 20 s into the *article fetch* — macOS entered forced "Maintenance Sleep", the network stack suspended (surfacing as bogus Yahoo read-timeouts), and GitHub cancelled the job with *"self-hosted runner lost communication with the server"*. `caffeinate -i` (idle-only) does **not** override Maintenance Sleep. The `-t 5400` timeout is a backstop so the lock self-expires if the job dies before the release step. A previous `scripts/update-pmset.sh` LaunchDaemon that set a Tue/Fri wake was removed as redundant.
